...
export default function QuestionBankCards({ jobRole, onDataChange }: QuestionBankCardsProps) {
  ...
  const [editIsComposing, setEditIsComposing] = useState(false)
  ...
  const executeEditSearch = useCallback(() => {
    setEditSearchText(editSearchText)
    setEditCurrentMatch(0)
    setEditIsComposing(false)
  }, [editSearchText])
  ...
  const handleSave = async (values: any) => {
    ...
    try {
      ...
      if (uploadMode === 'file' && fileList.length > 0) {
        ...
        if (fileObj && 'text' in fileObj && typeof fileObj.text === 'function') {
  ...
  useEffect(() => {
    if (uploadMode === 'manual' && !editIsComposing) {
      calculateEditMatches()
      setEditCurrentMatch(0)
    }
  }, [editSearchText, uploadMode, editIsComposing, calculateEditMatches])

  const handleEditNavigate = useCallback((direction: 'up' | 'down') => {
    if (editMatchCount === 0) return

    let newIndex: number
    if (direction === 'up') {
      newIndex = editCurrentMatch <= 0 ? editMatchCount - 1 : editCurrentMatch - 1
    } else {
      newIndex = editCurrentMatch >= editMatchCount - 1 ? 0 : editCurrentMatch + 1
    }
    setEditCurrentMatch(newIndex)

    const indices = calculateEditMatches()
    navigateToEditMatch(indices, newIndex)
  }, [editCurrentMatch, editMatchCount, calculateEditMatches, navigateToEditMatch])
                ...
                <div>
                  <div style={{ marginBottom: 12, display: 'flex', alignItems: 'center', gap: 12 }}>
                    <Input
                      ...
                      allowClear
                    />
                    <Button type="primary" onClick={() => setEditSearchText(editSearchText)}>搜索</Button>
                    {
                        editMatchCount > 0 && (
                          <span style={{ color: colors.textSecondary, fontSize: 12 }}>{editCurrentMatch + 1}/{editMatchCount}</span>
                        )
                      }
                    {editMatchCount > 0 && (
                      <Space>
